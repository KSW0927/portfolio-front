...
| 화면 | 경로 | 설명 |
|---|---|---|
| 로그인 | `/login` | 자동 로그인 버튼 클릭 시 시딩된 데모 계정으로 user-auth-service 로그인 API(`/api/users/login`)를 실제 호출하는 자동 로그인 |
| 대시보드 | `/main` | 주문 시뮬레이션 + 위젯 4종 + 재고 패널 |
...
이 프론트는 [NOTI-FLOW](https://github.com/KSW0927/portfolio-api) MSA 백엔드와 연동되어 동작하는 대시보드입니다. 로그인과 주문 처리는 REST로, 처리 결과 알림은 WebSocket으로 실시간 수신합니다.
...
| realtime-gateway-service (:8084) | WebSocket(`/ws` → `/topic/notifications` 구독) | `src/store/notifyStore.ts` |
...
React 18, TypeScript, Vite, Zustand, TanStack Query, Axios, @stomp/stompjs + SockJS, ag-grid-community
