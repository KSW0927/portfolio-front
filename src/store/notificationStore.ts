...
interface NotificationPublishedPayload {
 ...
 */
function rank(item: NotificationItem): number {
    if (item.category === '재고') return 2;
    if (item.pinned) return 1;
    return 0;
}

function insertSorted(list: NotificationItem[], item: NotificationItem): NotificationItem[] {
    return [item, ...list]
...
export const useNotificationStore = create<NotificationState>((set) => ({
    ...
    connect: () => {
        ...
        client = new Client({
            ...
            onConnect: () => {
                set({ connected: true });
                client?.subscribe(NOTIFICATIONS_TOPIC, (message: IMessage) => {
                    try {
                        const payload: NotificationPublishedPayload = JSON.parse(message.body);
