var ce_ImageEditorPlugin = {
	...
	start : function() {
		...
		if(CE_ItemManager.status.SELECTED_ITEM) {
			...
			//CROSS4-6395 레이어 안에서 포토에디터가 실행되지 않음
			//레이어, 테이블 안에서 포토에디터 실행되도록 조건 수정
			if (item.type !== 'image' && item.type !== 'layer' && item.type !== 'table') {
				return false;
			} else if(item.type !== 'layer' && item.type !== 'table'){
	...
	getEditorSetParameter : function () {
		...
		if (t._anchorNode != null) {
			...
			var iFloatStyleVal = t._anchorNode.style.cssFloat;
	...
	addWatermark: function(obj) {
		...
		var engine = t._oThis;
		if(!agentInfo.IsIE11 && engine.getWatermarkInfo() && engine.getWatermarkInfo().url && (engine.getWatermarkInfo().use == true || engine.getWatermarkInfo().use == "true")){
			
			var watermarkProcForBlob = function(selectedImage) {
				var watermarkSrc = engine.getWatermarkInfo().url;
				var watermarkOpacity = engine.getWatermarkInfo().opacity || 0.5;
				var watermarkPosition = engine.getWatermarkInfo().position || 1;
				var watermarkAngle = engine.getWatermarkInfo().rotate || 0;
				var watermarkWidth = engine.getWatermarkInfo().width;
				...
				if (selectedImage) {
					...
					reader.onload = function(event) {
						...
						img.onload = function() {
							...
							watermark.src = watermarkSrc; // 워터마크 이미지 경로를 지정하세요.
							watermark.onload = function() {
								//watermarkWidth = "33%"; // 설정할 워터마크 너비
								var watermarkHeight;
								if(!watermarkWidth || watermarkWidth == 0 || watermarkWidth == "0"){
									watermarkWidth = watermark.width;
									watermarkHeight = watermark.height;
								}else{
									if(watermarkWidth && watermarkWidth.indexOf("%") != -1){
										watermarkWidth = canvas.width * (parseInt(watermarkWidth) / 100);
									}else if(watermarkWidth && watermarkWidth.indexOf("px") != -1){
										watermarkWidth = parseInt(watermarkWidth);
									}
									watermarkHeight = watermark.height * (watermarkWidth / watermark.width); // 비율에 따른 높이 계산
								}

								var watermarkSpacing = watermarkWidth;
								if(watermarkWidth > watermarkHeight){
									watermarkSpacing = watermarkWidth;
								}else{
									watermarkSpacing = watermarkHeight;
								}
								// 워터마크 위치 계산
								var x, y;
								switch(watermarkPosition) {
									...
									break;
									case '2':
									x = canvas.width - watermarkWidth; //top-right
									y = 0;
									break;
									case '3': //bottom-left
									x = 0;
									...
									break;
									case '0': //repeat
									for (var i = 0; i < canvas.width; i += watermarkWidth + watermarkSpacing) {
										for (var j = 0; j < canvas.height; j += watermarkHeight + watermarkSpacing) {
							...
							watermark.onerror = function() {
								deferred.reject(NamoSELang.watermark_errorMsg);
							};
						};
					};
			...
			var obj2 = dataURItoBlob(obj);
			watermarkProcForBlob(obj2)
	...
	upload: function(obj) {
		var t = this;
		return this.addWatermark(obj).then(function (obj){
			return t.upload_old(obj);
		})
	},
	upload_old: function(obj) {
		var dataURItoBlob = function(dataURI) {
			// convert base64/URLEncoded data component to raw binary data held in a string
			var byteString;
			if (dataURI.split(',')[0].indexOf('base64') >= 0)
		...
		if (t._oThis.params.UploadFileExecutePath && t._oThis.params.UploadFileExecutePath.indexOf(t._oThis.baseHOST) != 0) {
			...
			if(t._oThis && t._oThis.config.EditorAuthConnection && t._oThis.config.EditorAuthConnection.toLowerCase() == "script"){
				t.unmaskParent();
				return false;
			}
		}
		...
		formData.append('imageEditorSource', 'true'); // 이미지 에디터에서 실행된 업로드 구분용
...
//			obj[0].name = tempFilename +'upload.png';
		...
		if (typeof t._oThis.params.event !== "undefined" && typeof t._oThis.params.event.UploadProc !== "undefined") {
			...
			dataObj['imageEditorSource'] = 'true'; // 이미지 에디터에서 실행된 업로드 구분용
			...
			if (typeof t._oThis.params.CsrfToken === 'string') {
				var idx = t._oThis.params.CsrfToken.indexOf(',');
				if (idx !== -1) {
					var headerName = t._oThis.params.CsrfToken.substring(0, idx).trim();
					var tokenValue = t._oThis.params.CsrfToken.substring(idx + 1).trim();
	...
	execute : function (data, t) {
		...
		//try{
			...
			if(item && data.mode==='ITEM') {
				...
				if(agentInfo.IsIE11 || agentInfo.IsSafari || agentInfo.IsGecko || agentInfo.IsChrome){
			...
			if(t._oThis.params.event){
				...
				if(typeof t._oThis.params.event !== "undefined" && typeof t._oThis.params.event.useCrossUploader !== "undefined"){
					var obj = {};
					obj.name = data.src.split("/")[data.src.split("/").length-1]
					obj.size = data.size;
					obj.path = data.src;
					t._oThis.params.event.useCrossUploader(obj);
				}
			}
		//} catch(e) {
			//console.log(e);
		...
		t.unmaskParent();
		t._oThis.setFormatEvent('ce_imageeditor');
